...
MAX_DEPTH = 2  # Puedes ajustar la profundidad para mejorar rendimiento

def evaluate_state(ai_pokemon: Pokemon, player_pokemon: Pokemon) -> int:
    """
    Evalúa un estado de combate: +PS de la IA - PS del jugador humano.
    """
    if is_fainted(player_pokemon):
        return 9999  # Victoria para la IA
    if is_fainted(ai_pokemon):
        return -9999  # Derrota para la IA
...
def minimax_decision(ai_pokemon: Pokemon, player_pokemon: Pokemon) -> int:
    """
    Devuelve el índice del mejor ataque para la IA.
    """
